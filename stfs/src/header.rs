...
fn serialize_byte_array<const N: usize, S: Serializer>(
    bytes: &[u8; N],
    s: S,
) -> Result<S::Ok, S::Error> {
...
fn serialize_byte_array_opt<const N: usize, S: Serializer>(
    bytes: &Option<[u8; N]>,
    s: S,
) -> Result<S::Ok, S::Error> {
    match bytes {
...
pub struct XContentHeader {
    ...
    #[serde(serialize_with = "serialize_byte_array_opt")]
...
/// Parse an STFS package header from raw bytes. Pure function, no I/O.
pub fn parse_header(input: &[u8]) -> Result<XContentHeader, StfsError> {
    ...
    let certificate = if let PackageType::Con = &package_type {
        Some(parse_certificate(&mut cursor)?)
    } else {
        None
    };
