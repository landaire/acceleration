...
use crate::hash::hash_table_skip_for_address;
...
impl StfsPackage {
    pub fn open<R: ReadAt>(source: &R) -> Result<Self, StfsError> {
        ...
        let header = parse_header(header_data.as_ref())?;
    ...
    fn extract_consecutive_blocks<R: ReadAt, W: Write>(
        ...
        let start_address =
            block_to_addr(entry.starting_block_num, self.sex, &self.hash_table_meta);
        ...
        if entry.block_count <= blocks_until_hash_table {
            ...
            while data_remaining > 0 {
                ...
                next_address +=
                    hash_table_skip_for_address(next_address, self.sex, &self.hash_table_meta);
            }
        }
    ...
    fn extract_chained_blocks<R: ReadAt, W: Write>(
        ...
        for _ in 0..block_count {
            ...
            let block_address = block_to_addr(block, self.sex, &self.hash_table_meta);

            let data = source.read_at(block_address..block_address + read_len)?;
            writer.write_all(data.as_ref())?;

            let hash_entry =
                read_block_hash_entry(source, block, self.sex, &self.hash_table_meta, stfs_vol)?;
