...
impl HashTableMeta {
    pub fn from_header(header: &XContentHeader, sex: StfsPackageSex) -> Result<Self, StfsError> {
        ...
        meta.top_table.true_block_number =
            meta.compute_backing_hash_block_number_for_level(0, top_level, sex);
        ...
        meta.top_table.entry_count =
            allocated_block_count / DATA_BLOCKS_PER_HASH_TREE_LEVEL[top_level as usize];
    ...
    pub fn read_top_table<R: ReadAt>(&mut self, source: &R) -> Result<(), StfsError> {
        ...
        for _ in 0..self.top_table.entry_count {
            ...
            self.top_table.entries.push(HashEntry {
                block_hash,
                status,
                next_block,
            });
...
pub fn block_to_addr(block: usize, sex: StfsPackageSex, hash_meta: &HashTableMeta) -> usize {
    if block > 2usize.pow(24) - 1 {
        panic!("invalid block: {:#x}", block);
    }

    (compute_data_block_num(block, sex) * BLOCK_SIZE) + hash_meta.first_table_address
}

pub fn compute_data_block_num(block: usize, sex: StfsPackageSex) -> usize {
    let addr =
        (((block + HASHES_PER_HASH_TABLE) / HASHES_PER_HASH_TABLE) << (sex as usize)) + block;
    if block < HASHES_PER_HASH_TABLE {
        addr
    } else if block < DATA_BLOCKS_PER_HASH_TREE_LEVEL[2] {
        (addr + ((addr + DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]) / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]))
            << sex as usize
    } else {
        (1 << sex as usize)
            + ((addr
...
pub fn hash_table_skip_for_address(
    ...
    let mut block_number = (table_address - hash_meta.first_table_address) / BLOCK_SIZE;

    if block_number == 0 {
        return BLOCK_SIZE << sex as usize;
    }

    if block_number == hash_meta.block_step[1] {
        return 0x3000 << sex as usize;
    } else if block_number > hash_meta.block_step[1] {
        block_number -= hash_meta.block_step[1] + (1 << sex as usize);
    }

    if block_number == hash_meta.block_step[0]
        || block_number.is_multiple_of(hash_meta.block_step[1])
    {
...
pub fn read_block_hash_entry<R: ReadAt>(
    ...
    if block > stfs_vol.allocated_block_count as usize {
        return Err(StfsError::InvalidBlock {
            block,
            allocated: stfs_vol.allocated_block_count as usize,
        });
    }

    let addr = block_hash_address(block, sex, hash_meta, stfs_vol, source)?;
    ...
    cursor.read_exact(&mut block_hash)?;
    let status = cursor.read_u8()?;
    let next_block = cursor.read_u24::<BigEndian>()? as u32;
...
pub fn block_hash_address<R: ReadAt>(
    ...
    let mut hash_addr = (hash_meta.compute_first_level_backing_hash_block_number(block, sex)
        * BLOCK_SIZE)
        + hash_meta.first_table_address;
    hash_addr += (block % HASHES_PER_HASH_TABLE) * 0x18;

    match hash_meta.top_table.level {
        HashTableLevel::First => {
            Ok(hash_addr + (((stfs_vol.block_separation as usize) & 2) << 0xB))
        }
        HashTableLevel::Second => {
            let entry = &hash_meta.top_table.entries[block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]];
            Ok(hash_addr + (((entry.status as usize) & 0x40) << 6))
        }
        HashTableLevel::Third => {
            let first_level_offset = ((hash_meta.top_table.entries
                [block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]]
                .status as usize)
                & 0x40)
                << 6;

            let position = (hash_meta.compute_second_level_backing_hash_block_number(block, sex)
                * BLOCK_SIZE)
                + hash_meta.first_table_address
                + first_level_offset
                + ((block % DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]) * 0x18);

            // Read the status byte at position + 0x14
