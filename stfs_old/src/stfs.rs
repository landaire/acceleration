...
fn input_byte_ref<'a>(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8], size: usize) -> &'a [u8] {
	let position: usize = cursor.position().try_into().expect("failed to convert position to usize");
	cursor.set_position((position + size).try_into().expect("failed to convert pos into usize"));
...
fn read_utf16_cstr<'a>(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> String {
	...
	cursor.set_position((position + end_of_str_position + 2).try_into().expect("failed to convert pos into usize"));
...
fn read_utf8_with_max_len<'a>(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8], len: usize) -> String {
	let position: usize = cursor.position().try_into().expect("failed to convert position to usize");
	...
	cursor.set_position((position + len).try_into().expect("failed to convert pos into usize"));
	let byte_range = &input[position..end_of_str_position];
...
impl<'a> HashTableMeta<'a> {
	pub fn parse(data: &'a [u8], sex: StfsPackageSex, header: &XContentHeader) -> Result<Self, StfsError> {
...
impl<'a> StfsPackage<'a> {
	pub fn extract_file<W: Write>(&self, writer: &mut W, entry: &StfsFileEntry) -> std::io::Result<()> {
		...
		reader.read_to_end(&mut data).expect("failed to read STFS file");
		writer.write_all(data.as_slice()).expect("failed to write to file output");
	...
	fn block_hash_entry(&self, block: usize, input: &'a [u8]) -> HashEntry {
		...
		if block > stfs_vol.allocated_block_count as usize {
			panic!("Reference to illegal block number: {:#x} ({:#x} allocated)", block, stfs_vol.allocated_block_count);
		}
	...
	fn block_hash_address(&self, block: usize, input: &'a [u8]) -> u64 {
		...
		let mut hash_addr = (self.hash_table_meta.compute_first_level_backing_hash_block_number(block, self.sex)
			* BLOCK_SIZE)
		...
		match self.hash_table_meta.top_table.level {
			HashTableLevel::First => hash_addr as u64 + (((stfs_vol.block_separation as u64) & 2) << 0xB),
			HashTableLevel::Second => {
				hash_addr as u64
					+ ((self.hash_table_meta.top_table.entries[block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]].status
						as u64 & 0x40) << 6)
			}
			HashTableLevel::Third => {
				let mut reader = Cursor::new(input);
				let first_level_offset =
					(self.hash_table_meta.top_table.entries[block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]].status as u64
						& 0x40) << 6;

				let position = (self.hash_table_meta.compute_second_level_backing_hash_block_number(block, self.sex)
	...
	fn read_files(&mut self, input: &'a [u8]) {
		...
		for block_idx in 0..(stfs_vol.file_table_block_count as usize) {
			...
			for file_entry_idx in 0..0x40 {
				...
				let name_len = reader
					.read_u8()
					.unwrap_or_else(|_| panic!("failed to read name_len at {:#x}", entry.file_entry_address));
				if name_len & 0x3F == 0 {
				...
				entry.block_count = reader.read_u24::<LittleEndian>().expect("failed to read blocks_for_file") as usize;

				reader.set_position(reader.position() + 3);

				entry.starting_block_num =
					reader.read_u24::<LittleEndian>().expect("failed to read blocks_for_file") as usize;
				entry.path_indicator = reader.read_u16::<BigEndian>().expect("failed to read blocks_for_file");
				entry.file_size = reader.read_u32::<BigEndian>().expect("failed to read file_size") as usize;
				entry.created_time_stamp = reader.read_u32::<BigEndian>().expect("failed to read created_time_stamp");
				entry.access_time_stamp = reader.read_u32::<BigEndian>().expect("failed to read access_time_stamp");
		...
		for file in files.drain(..) {
			if let StfsEntry::File(entry) | StfsEntry::Folder { entry, files: _ } = &*file.lock() {
				...
				if let Some(entry) = cached_entry {
					...
					panic!("Corrupt STFS file: missing folder index {:#x}", entry.path_indicator);
				}
			}
		}
	...
	fn compute_data_block_num(&self, block: usize) -> u64 {
		let addr = ((((block + HASHES_PER_HASH_TABLE) / HASHES_PER_HASH_TABLE) << (self.sex as usize)) + block) as u64;
		if block < HASHES_PER_HASH_TABLE {
			addr
		} else if block < DATA_BLOCKS_PER_HASH_TREE_LEVEL[2] {
			addr + (((addr + DATA_BLOCKS_PER_HASH_TREE_LEVEL[2] as u64) / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2] as u64)
				<< self.sex as usize)
		} else {
			((1 << self.sex as usize)
				+ ((addr as usize
					+ ((block + DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]) / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]))
...
fn certificate_parser<'a>(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> Result<Certificate<'a>, StfsError> {
	...
	let owner_console_part_number = std::str::from_utf8(
		&owner_console_part_number
			[..owner_console_part_number.iter().position(|b| *b == 0x0).unwrap_or(owner_console_part_number.len())],
	)
...
fn xcontent_header_parser<'a>(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> Result<XContentHeader<'a>, StfsError> {
	let mut package_type = [0u8; 4];
	cursor.read_exact(&mut package_type)?;
	let package_type = PackageType::try_from(package_type)?;

	let certificate =
		if let _package_type = PackageType::Con { Some(certificate_parser(cursor, input)?) } else { None };

	let (input, package_signature) = if matches!(package_type, PackageType::Live | PackageType::Pirs) {
	...
	for i in 0..license_data.len() {
		...
		license_data[i].ty =
			LicenseType::try_from(u16::try_from(license >> 48).expect("failed to convert license type to u16"))
	...
	let content_metadata = match content_type {
		ContentType::AvatarItem => {
			...
			Some(ContentMetadata::AvatarItem(AvatarAssetInformation::parse(cursor, input)?))
		}
		ContentType::Video => {
			cursor.set_position(0x3d9);
			Some(ContentMetadata::Video(MediaInformation::parse(cursor, input)?))
		}
		_ => None,
	};
...
impl<'a> AvatarAssetInformation<'a> {
	fn parse(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> Result<AvatarAssetInformation<'a>, StfsError> {
...
impl<'a> StfsVolumeDescriptor<'a> {
	fn parse(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> Result<StfsVolumeDescriptor<'a>, StfsError> {
...
impl<'a> SvodVolumeDescriptor<'a> {
	fn parse(cursor: &mut Cursor<&'a [u8]>, input: &'a [u8]) -> Result<SvodVolumeDescriptor<'a>, StfsError> {
